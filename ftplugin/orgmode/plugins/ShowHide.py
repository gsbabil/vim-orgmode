...
class ShowHide(object):
    ...
    def _fold_depth(cls, h):
        ...
        :h:            Heading
        :returns:    Tuple (int - level of open folds, boolean -
        found fold) or None if h is not a Heading
        """
    ...
    def toggle_folding(cls, reverse=False):
        ...
        :reverse:    If False open folding by one level otherwise close
        it by one.
        """
        ...
        if int(vim.eval(u_encode(u'foldclosed(%d)' %
                                 heading.start_vim))) != -1:
            if not reverse:
                # open closed fold
                ...
                vim.command(u_encode(u'%d,%dfoldopen!' %
                                     (heading.start_vim,
        ...
        def open_fold(h):
            if h.number_of_parents <= open_depth:
                vim.command(u_encode(u'normal! %dgg%dzo' % (h.start_vim,
                                                            open_depth)))
            for c in h.children:
                open_fold(c)
        ...
        if not reverse:
            ...
            if found_fold:
                ...
                vim.command(u_encode(u'%d,%dfoldclose!' %
                                     (heading.start_vim,
                                      heading.end_of_last_child_vim)))

                if heading.number_of_parents:
                    # restore cursor position, it might have been
                    # changed by open_fold
                    vim.current.window.cursor = cursor

                    p = heading.number_of_parents
                    if not p:
                        p = heading.level
                    # reopen fold again beacause the former closing of
                    # the fold closed all levels, including parents!
                    vim.command(u_encode(u'normal! %dzo' % (p, )))
        else:
    ...
    def global_toggle_folding(cls, reverse=False):
        ...
        if reverse:
            ...
            if foldlevel == 0:
                # open all folds because the user tries to close folds
                # beyound 0
    ...
    def register(self):
        u"""
        Registration of plugin. Key bindings and other initialization
        should be done.
        """
        # register plug
        ...
        for i in range(0, 10):
            self.keybindings.append(Keybinding(u'<localleader>%d' % (i,
                                                                     ),
                                               u'zM:set fdl=%d<CR>' % i,
                                               mode=MODE_NORMAL))
